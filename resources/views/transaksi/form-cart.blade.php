...
<script>
    ...
    function addRow(item) {
        const {
            ...
            title,
            quantity,
            price, // Ini sudah harga setelah diskon
            total_price,
            options
        } = item;
        ...
        // Ambil data dari options
        const { diskon, harga_jual } = options;
        
        // Tampilkan harga dengan format yang jelas
        let harga_display = '';
        if (diskon > 0) {
            harga_display = `<div>
                <span style="text-decoration: line-through; color: #999; font-size: 0.9em;">Rp ${rupiah(harga_jual)}</span><br>
                <strong style="color: #28a745;">Rp ${rupiah(price)}</strong> 
                <small class="text-success">(-${diskon}%)</small>
            </div>`;
        } else {
            harga_display = `<strong>Rp ${rupiah(price)}</strong>`;
        }
        ...
        <td>${quantity}</td>
        <td>${harga_display}</td>
        <td><strong>Rp ${rupiah(total_price)}</strong></td>
        <td>${btn}</td>
        </tr>`;
